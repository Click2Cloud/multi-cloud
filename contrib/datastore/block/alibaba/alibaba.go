...
// These functions are use to interact with alibaba cloud through their exposed API's
...
func (ad *AlibabaAdapter) CreateVolume(ctx context.Context, volume *block.CreateVolumeRequest) (*block.CreateVolumeResponse, error) {
	...
	// request body of the API CreateDisk of alibaba, where we are sending input parameters required to create volume
...
func (ad *AlibabaAdapter) GetVolume(ctx context.Context, volume *block.GetVolumeRequest) (*block.GetVolumeResponse, error) {
	...
	//vol := &block.Volume{
	//	Name:               response.Disks.Disk[0].DiskName,
	//	Description:        response.Disks.Disk[0].Description,
	//	Size:               (int64)(response.Disks.Disk[0].Size),
	//	Type:               response.Disks.Disk[0].Type,
	//	Region:             response.Disks.Disk[0].RegionId,
	//	AvailabilityZone:   response.Disks.Disk[0].ZoneId,
	//
	//}
...
//input.Tag = &tags
//tag := []ecs.CreateDiskTag{}
//tag = append(tag, ecs.CreateDiskTag{
//Key: volume.Volume.Tags[0].Value,
//Value:volume.Volume.Tags[0].Value,
//
//resource type = disk
//
//})
