...
func (adapter *mongoAdapter) ListVolume(ctx context.Context, limit, offset int, query interface{}) ([]*model.Volume, error) {
...
func (adapter *mongoAdapter) GetVolume(ctx context.Context, id string) (*model.Volume, error) {
	session := adapter.session.Copy()
	defer session.Close()
