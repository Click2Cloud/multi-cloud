...
func handleOssOrangeErrors(err error) error {
	if err != nil {
...
func (mover *S3Mover) UploadObj(objKey string, destLoca *LocationInfo, buf []byte) error {
	...
	s3c := OssCred{ak: destLoca.Access, sk: destLoca.Security}
	...
	if err != nil {
		log.Errorf("[Ossmover] New session failed, err:%v\n", err)
		return handleOssOrangeErrors(err)
	}

	reader := bytes.NewReader(buf)
	uploader := s3manager.NewUploader(sess)
	log.Infof("[Ossmover] Try to upload, bucket:%s,obj:%s\n", destLoca.BucketName, objKey)
	...
	for tries := 1; tries <= 3; tries++ {
		...
		if err != nil {
			log.Errorf("[Ossmover] Upload object[%s] failed %d times, err:%v\n", objKey, tries, err)
			e := handleOssOrangeErrors(err)
			if tries >= 3 || e.Error() == DMERR_NoPermission { //If no permission, then no need to retry.
				return e
			}
		} else {
			log.Infof("[Ossmover] Upload object[%s] successfully.", objKey)
			return nil
		}
	}
...
func (mover *S3Mover) DownloadObj(objKey string, srcLoca *LocationInfo, buf []byte) (size int64, err error) {
	s3c := OssCred{ak: srcLoca.Access, sk: srcLoca.Security}
	creds := credentials.NewCredentials(&s3c)
	sess, err := session.NewSession(&aws.Config{
	...
	if err != nil {
		...
		return 0, handleOssOrangeErrors(err)
	}
...
func ListObjs(loca *LocationInfo, filt *pb.Filter) ([]*s3.Object, error) {
	...
	if err != nil {
		log.Errorf("[s3mover] new session failed, err:%v\n", err)
		return nil, handleOssOrangeErrors(err)
	}
